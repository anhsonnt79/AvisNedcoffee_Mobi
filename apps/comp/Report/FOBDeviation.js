...
export default class FOBDeviation extends React.Component {
  constructor(props){
    super(props);
    var from_day = '01';
  ...
  fetchDataFromApi = ()  => {
    ...
    fetch(global.URL + '/outbound/fobdeviation', {
      method: 'POST',
      body: form
      })
                    ...
                    //   fromdate: this.state.from_date, todate: this.state.to_date})}
                        ...
                        <View style={{ justifyContent: 'space-between', flexDirection: 'row', borderBottomWidth: 0.5, borderTopWidth:0 }}>                      
                            <View style={{ justifyContent: 'space-between', flexDirection: 'column' }}>
                                <Text style={styles.menuText}>DO No.: </Text>
                                <Text style={styles.menuText}>{item.do_no} </Text>
                            </View>
                            <View style={{ justifyContent: 'space-between', flexDirection: 'column' }}>
                                <Text style={styles.menuText}> DO Date: </Text>
                                <Text style={styles.menuText}> {item.fob_date} </Text>
                            </View>
                            <View style={{ justifyContent: 'space-between', flexDirection: 'column' }}>
                                <Text style={styles.menuText}> SI No.: </Text>
                                <Text style={styles.menuText}> {item.si_no} </Text>
                            </View>
                            <View style={{ justifyContent: 'space-between', flexDirection: 'column' }}>
                                <Text style={styles.menuText}> NVS-NLS:</Text>
                                <Text style={styles.menuText}> {item.con_name}</Text>
                            </View>
                        </View>
                        <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                            <Text style={styles.menuText}>GDN No.: </Text>
                            <Text style={styles.menuText}>{item.pickingname} </Text>
                        </View>
                        <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                            <Text style={styles.menuText}>Weight Factory (Kg): </Text>
                            <Text style={styles.menuText}>{item.weightfactory.toLocaleString()} Kg</Text>
                        </View>
                        <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                            <Text style={styles.menuText}>Weight HCM (Kg): </Text>
                            <Text style={styles.menuText}>{item.shipped_weight.toLocaleString()} Kg</Text>
                        </View>
                        <View style={{ justifyContent: 'space-between', flexDirection: 'row' }}>
                            <Text style={styles.menuText}>Claim (>0.1% kg): </Text>
                            <Text style={styles.menuText}>{item.claim_percent.toLocaleString()}  %</Text>
                        </View>
                    </View>}            
